...
CREATE TABLE dwh.customer_report_datamart (
    ...
    top_month_product_category int8 NULL, -- самая популярная категория товаров у этого заказчика за месяц
    -- Если клиент не совершал заказов в течении месяца, то категория продукта = 'Null'

    top_craftsman_id int8 NULL, -- идентификатор самого популярного мастера ручной работы у заказчика. Если заказчик сделал одинаковое количество заказов у нескольких мастеров, возьмите любого
    -- Если клиент не совершал заказов в течении месяца, то мастер = 'Null'
